...
class CSTri(object):
    ...
    def edge_polygon(self):
        ...
        if pnew is not None and  not self.check_alpha_lim(grd_s, pnew, False):
    ...
    def _distance_test(
        ...
        for item in self.ale:
            ...
            if item not in exceptions and  is_circle_cross_segment(center,r,item.start,item.end) and np.dot(e_grd,
                    self.func_gd(pt)) > 0:
                if dis<r and (min_dis is None or dis<min_dis):
                    min_dis = dis
                    emin = item
                else:
                    dis2=vec_len(e.start-center)
                    if dis2 <dis :
                        dis=dis2
                        item=item.pred
    ...
    def edge_spining(
        ...
        vnew = vinit * r2
        pnew = p_s + vnew
        rot_mat = rotation_array(self.dlt_alpha, d12)
    ...
    def _search_root_on_plane(self, pnew, plane_norm, normed=True):
        ...
        grdnew = projection_to_plan(grdnew, plane_norm)
        func_val = self.func(pnew)
    ...
    def check_alpha_lim(self, grd, pnew, normed=True):
        if normed:
            ...
            if intersec_angle(grdnew, grd, True) < self.alpha_lim:
                return True
        else:
            grdnew = self.func_gd(pnew)
            if intersec_angle(grdnew, grd) < self.alpha_lim:
                return True
        return False
    ...
    def first_triangle(self, p0, c):
        ...
        p1 = self.edge_spining(p0, p0 + t*c, n, c)
